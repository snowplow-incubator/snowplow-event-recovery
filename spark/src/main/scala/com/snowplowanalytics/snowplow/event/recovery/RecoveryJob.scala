...
trait RecoveryJob {
  ...
  private[this] def init(): SparkSession = {
    ...
    val session      = SparkSession.builder().config(conf).config("*.source.metrics.class", "org.apache.spark.metrics.source.Metrics").getOrCreate()
...
case class Summary(successful: LongAccumulator, unrecoverable: LongAccumulator, failed: LongAccumulator) {
  def this(sc: SparkContext) = this(sc.longAccumulator("recovered"), sc.longAccumulator("unrecoverable"), sc.longAccumulator("failed"))
}
