...
project_version=$(sbt beam/version -Dsbt.log.noformat=true | perl -ne 'print "$1\n" if /info.*(\d+\.\d+\.\d+[^\r\n]*)/' | tail -n 1 | tr -d '\n')
if [[ "${tag}" = *"${project_version}" ]]; then
    sbt "project beam" docker:publishLocal
    docker tag "snowplow-event-recovery-beam:${tag}" "${docker_repo}/snowplow/snowplow-event-recovery-beam:${tag}"
    docker push "${docker_repo}/snowplow/snowplow-event-recovery-beam:${tag}"
else
