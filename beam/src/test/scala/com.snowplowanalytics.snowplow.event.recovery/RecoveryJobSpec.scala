...
import com.spotify.scio.io.{TextIO, PubsubIO}
...
class RecoveryJobSpec extends PipelineSpec {
  ...
  "Recovery" should "fix a set of bad rows according to a set of recovery scenarios" in {
    JobTest[Main.type]
      .args("--inputDirectory=in", "--outputTopic=out", s"--config=$recoveryScenarios",  s"--resolver=$resolver")
