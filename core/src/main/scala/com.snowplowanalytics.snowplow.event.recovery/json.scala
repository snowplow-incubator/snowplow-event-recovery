...
object json {
  ...
  implicit val applyE: Encoder[Apply.type] =
    Encoder.encodeString.contramap[Apply.type](_.toString)
  implicit val applyD: Decoder[Apply.type] = Decoder.decodeString.emap {
    case "Apply" => Right(Apply)
    case _       => Left("Apply")
  }
  ...
  implicit val matcherE: Encoder[conditions.Matcher] = Encoder.instance {
    ...
    case r: conditions.Function          => r.asJson
  ...
  implicit val applicationE: Encoder[Application] = deriveEncoder
  implicit val applicationD: Decoder[Application] = deriveDecoder
