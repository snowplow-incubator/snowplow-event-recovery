...
case class UnableToDecodeBase64String(data: String) extends RecoveryStatus {
  def message = s"Failed to properly decode Base64-encoded string: $data"
}

object UnableToDecodeBase64String {
  implicit val encoder: Encoder[UnableToDecodeBase64String] = deriveEncoder
}
