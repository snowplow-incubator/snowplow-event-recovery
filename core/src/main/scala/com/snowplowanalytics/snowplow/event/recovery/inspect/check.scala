...
object check {
  ...
  /**
    ...
    *
    * @param matcher an instance of value matcher to use
  ...
  def apply(matcher: Matcher)(path: Seq[String])(body: Json): Recovering[Json] =
    transform(
      ...
      (_: ACursor) => _.focus.toRight(ComparisonFailure(path.mkString("."), matcher.asJson.noSpaces, body.noSpaces)),
      ComparisonFailure(path.mkString("."), matcher.asJson.noSpaces, _)
    )(path)(body)
